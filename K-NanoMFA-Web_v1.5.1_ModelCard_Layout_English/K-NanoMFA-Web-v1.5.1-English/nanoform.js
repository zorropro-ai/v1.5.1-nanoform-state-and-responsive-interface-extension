(function(root,factory){
  ...
  function refresh(){if(app?.state?.result)onModelResult(app.state.result,{mode:app.state.mode,releaseForms:app.deepClone(app.state.releaseForms),environment:app.deepClone(app.state.environment),region:app.deepClone(app.currentRegion()),material:app.materialDisplayName()});else renderResults();}
  ...
  function initUI(application){
    ...
    $('downloadNanoformCsvBtn')?.addEventListener('click',()=>{if(!lastOutput){alert('Run the model with nanoform state tracking enabled before exporting.');return;}app?.download?.('K-NanoMFA_nanoform_state_inventory_v15.csv',csvText(),'text/csv;charset=utf-8');});
    renderAll();refresh();
  }
