...
const APP_INFO=Object.freeze({
  name:'K-NanoMFA',version:'1.5.1-en',initial_release_date:'2026-07-17',current_release_date:'2026-07-22',
...
function init(){
  ...
  try{interfaceView=localStorage.getItem('K-NanoMFA-v15-interface-view')||'guided';}catch(_){interfaceView='guided';}setInterfaceView(interfaceView,false);setMode('static_deterministic');
  let saved=null;try{saved=localStorage.getItem('K-NanoMFA-v15-en')||localStorage.getItem('K-NanoMFA-v14-en')||localStorage.getItem('K-NanoMFA-v13-en')||localStorage.getItem('K-NanoMFA-v121-en')||localStorage.getItem('K-NanoMFA-v12-en')||localStorage.getItem('K-NanoMFA-v10-en')||localStorage.getItem('K-NanoMFA-v092-en')||localStorage.getItem('K-NanoMFA-v09-en')||localStorage.getItem('K-NanoMFA-v08-en')||localStorage.getItem('K-NanoMFA-v07-en');}catch(e){console.warn('Browser storage is unavailable; scenarios can still be exported as JSON.',e);}
...
function renderReleaseForms(){
  ...
  $('releaseFormTable').querySelectorAll('[data-release-form]').forEach(el=>el.addEventListener('change',()=>{const [m,k]=el.dataset.releaseForm.split('.');state.releaseForms[m][k]=Number(el.value);renderReleaseForms();renderReleaseFormResults();window.KNanoNanoform?.refresh?.();validate();}));
}
...
function fitnessFlagsData(){
  ...
  flags.push({level:'warn',text:'PECs are screening calculations and do not resolve mechanistic aggregation, dissolution, deposition, resuspension, or intermedia transfer.'});
...
function calculate(){
  ...
  try{
    ...
    PI?.renderResult?.(state.result);state.mc=null;$('mcResults').hidden=true;$('emptyResults').hidden=true;$('resultsArea').hidden=false;renderReleaseFormResults();window.KNanoNanoform?.onModelResult?.(state.result,{mode:state.mode,releaseForms:deepClone(state.releaseForms),environment:deepClone(state.environment),region:deepClone(currentRegion()),material:materialDisplayName()});renderFitnessFlags();renderComparison();renderProvenance();renderBenchmark();renderWorkflowGuide();window.KNanoAdvanced?.onModelResult?.(state.result);switchTab('results');
...
function scenario(){readMetadata();if(isCustomMaterial()){readCustomIdentity(false);registerCustomRuntime();}return {app:'K-NanoMFA',version:'1.5.1-en',software_metadata:deepClone(APP_INFO),language:'English',interface_view:interfaceView,default_provenance:provenanceRows(),advanced:window.KNanoAdvanced?.getState?.()||null,mixture:window.KNanoMixture?.getState?.()||null,product_inventory:PI?.getState?.()||null,nanoform:window.KNanoNanoform?.getState?.()||null,mode:state.mode,material:state.material,material_display_name:materialDisplayName(),customMaterial:isCustomMaterial()?deepClone(state.customMaterial):null,custom_geography:window.KNanoGeography?.getDefinition?.(currentRegion().country_code)||null,country_code:currentRegion().country_code,domain_id:currentRegion().domain_id,region:currentRegion().region,geography:state.geography,totalMass:Number($('totalMass').value),baseYear:Number($('baseYear').value),metadata:state.metadata,qualityProfile:state.qualityProfile,uncertaintyProfile:uncertaintyProfileMap(),releaseForms:state.releaseForms,products:state.products,eol:state.eol,factors:state.factors,sludge:state.sludge,lifetimes:state.lifetimes,environment:state.environment,dynamicSettings:state.dynamicSettings,trajectory:state.trajectory};}
...
function setInterfaceView(view,persist=true){interfaceView=view==='all'?'all':'guided';if(persist){try{localStorage.setItem('K-NanoMFA-v15-interface-view',interfaceView);}catch(_){}}updateWorkspaceNavigation();}
...
function bind(){
  ...
  $('resetReleaseFormsBtn').onclick=()=>{state.releaseForms=isCustomMaterial()?customDefaultBlock('releaseForms',state.releaseForms):deepClone(D.RELEASE_FORM_DEFAULTS?.[state.material]||D.RELEASE_FORM_DEFAULTS?.CNT||{});renderReleaseForms();renderReleaseFormResults();window.KNanoNanoform?.refresh?.();validate();};
  ...
  $('saveLocalBtn').onclick=()=>{try{localStorage.setItem('K-NanoMFA-v15-en',JSON.stringify(scenario()));alert('Scenario saved in this browser.');}catch(e){alert('Browser storage is unavailable. Use Export scenario JSON instead.');}};
  $('exportScenarioBtn').onclick=()=>download(`K-NanoMFA_${safeFilePart(materialDisplayName())}_${state.mode}_scenario_v15.json`,JSON.stringify(scenario(),null,2),'application/json');
  ...
  $('downloadResultsBtn').onclick=()=>download(`K-NanoMFA_${safeFilePart(materialDisplayName())}_${state.mode}_audit_v15.json`,JSON.stringify({...scenario(),fitness_flags:fitnessFlagsData(),form_resolved_releases:formResolvedRows(),result:state.result,uncertainty:state.mc,sensitivity:state.sensitivity,comparison_runs:state.comparisons,benchmark_validation:benchmarkAuditData(),advanced_outputs:window.KNanoAdvanced?.getOutputs?.()||null,mixture_outputs:window.KNanoMixture?.getOutputs?.()||null,nanoform_outputs:window.KNanoNanoform?.getOutputs?.()||null},null,2),'application/json');
  $('downloadFateInventoryBtn').onclick=()=>{if(!state.result){alert('Run the model before exporting the fate-input inventory.');return;}download(`K-NanoMFA_${safeFilePart(materialDisplayName())}_${safeFilePart(currentRegion().domain_id)}_fate_input_v15.csv`,fateInventoryCsv(),'text/csv;charset=utf-8');};
